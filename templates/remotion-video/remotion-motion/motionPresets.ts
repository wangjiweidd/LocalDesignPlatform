...
export const beatStartFrame = (beat: VisualBeat) => {
  // The current renderer can rely on frame. A caption-aware renderer can replace this
  // with anchor timing plus offsetFrames without changing the beat contract.
  return Math.max(0, Math.round(beat.frame + (beat.offsetFrames ?? 0)));
};

export const motionStyleForBeat = (beat: VisualBeat, frame: number): CSSProperties => {
  const preset = beat.preset ?? fallbackPresetForAction(beat.action);
  const defaults = motionPresetDefaults[preset];
  const start = beatStartFrame(beat);
...
const fallbackPresetForAction = (action: LegacyBeatAction): MotionPresetId => {
  switch (action) {
    case 'pop':
      return 'soft-pop';
    case 'draw':
      return 'hand-drawn-check';
    case 'pulse':
      return 'boundary-pulse';
    case 'shake':
      return 'warning-nudge';
    case 'slide':
      return 'gentle-slide';
    case 'flash':
